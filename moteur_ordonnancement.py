...
def charger_donnees(params):
    ...
    art=pd.read_csv(d+"articles.csv",sep=sep,encoding=enc,skipinitialspace=True)
    art=art.loc[:,~art.columns.str.startswith("Unnamed")]
    art.columns=["itmref","designation","categorie","type_appro","delai"]
    ...
    stk=pd.read_csv(d+"stock.csv",sep=sep,encoding=enc,skipinitialspace=True)
    stk=stk.loc[:,~stk.columns.str.startswith("Unnamed")]
    stk.columns=["itmref","stock_physique","stock_alloue","stock_bloque"]
    ...
    cmd=pd.read_csv(d+"commandes_clients.csv",sep=sep,encoding=enc)
    cmd=cmd.loc[:,~cmd.columns.str.startswith("Unnamed")]
    cmd.columns=["sohnum","soplin","client_code","client_nom","itmref","designation",
    ...
    of=pd.read_csv(d+"of_entetes.csv",sep=sep,encoding=enc)
    of=of.loc[:,~of.columns.str.startswith("Unnamed")]
    of.columns=["mfgnum","itmref","designation","mfgsta","mfgsta_lib",
    ...
    comp=pd.read_csv(d+"of_composants.csv",sep=sep,encoding=enc)
    comp=comp.loc[:,~comp.columns.str.startswith("Unnamed")]
    comp.columns=["mfgnum","composant","designation","qty_requise","dat_besoin"]
    ...
    oa=pd.read_csv(d+"receptions_oa.csv",sep=sep,encoding=enc)
    oa=oa.loc[:,~oa.columns.str.startswith("Unnamed")]
    oa.columns=["pohnum","itmref","fournisseur","qte_restante","date_reception"]
    ...
    gam=pd.read_csv(d+"gammes.csv",sep=sep,encoding=enc,skipinitialspace=True)
    gam=gam.loc[:,~gam.columns.str.startswith("Unnamed")]
    gam.columns=["itmref","poste_charge","libelle_poste","cadence"]
            ...
            if of_enfant_num:
                ...
                if dates_c:
                    ...
                    if pd.notna(dat_besoin_comp) and pd.notna(date_dispo_sf):
                        d1=date_dispo_sf.to_pydatetime() if isinstance(date_dispo_sf,pd.Timestamp) else date_dispo_sf
                        d2=dat_besoin_comp.to_pydatetime() if isinstance(dat_besoin_comp,pd.Timestamp) else dat_besoin_comp
                        delta=(d1-d2).days
...
def calculer_plan_charge(dfs, resultats_of, params):
    ...
    # Jours ouvres de l'horizon
    date_lim=date_ref+timedelta(days=horizon)
    jours_ouvres=generer_jours_ouvres(date_ref, date_lim, FERMETURES_AERECO)
    if not jours_ouvres: return pd.DataFrame(), {}
    ...
    for mfgnum in of_a_charger:
        ...
        if poste not in charge_map:
            ...
            for j in jours_ouvres:
                charge_map[poste][j]={"charge":0.0,"of_list":[],"allocations":[]}

        # Backscheduling : placer la charge au plus tard
        if isinstance(date_fin_of, pd.Timestamp): date_fin_of=date_fin_of.to_pydatetime()
        # Trouver le dernier jour ouvre <= date_fin_of dans l'horizon
        jours_dispo=[j for j in jours_ouvres if j<=date_fin_of]
        if not jours_dispo:
            # Date fin dans le passe ou hors horizon -> charger sur le premier jour
            jours_dispo=[jours_ouvres[0]]

        charge_restante=charge_h
        date_debut_charge=None
        date_fin_charge=None
        for jour in reversed(jours_dispo):
            if charge_restante<=0: break
        ...
        # Si charge_restante > 0 : surcharge non resolvable, on empile sur le premier jour
        if charge_restante>0:
            premier_jour=jours_ouvres[0]
            if premier_jour not in charge_map[poste]:
                charge_map[poste][premier_jour]={"charge":0.0,"of_list":[],"allocations":[]}
            charge_map[poste][premier_jour]["charge"]+=charge_restante
            charge_map[poste][premier_jour]["of_list"].append(mfgnum)
            charge_map[poste][premier_jour]["allocations"].append({"mfgnum":mfgnum,"charge_h":charge_restante})
            date_debut_charge=premier_jour
    ...
    for mfgnum,info in of_capa.items():
        if info["date_debut"] and info["date_fin"] and info["poste"]:
            ...
            jours_of=[j for j in jours_ouvres if d1<=j<=d2]
            semaines_of=set()
            for j in jours_of:
                iso=j.isocalendar()
...
def assembler(dfs, resultats_of, of_capa=None):
    ...
    for idx in cmd.index:
        ...
        if on and not of_dates_idx.empty and on in of_dates_idx.index and pd.notna(date_besoin_prod):
            ...
            if pd.notna(date_fin_of) and date_fin_of>date_besoin_prod:
                retard_j=(date_fin_of-date_besoin_prod).days
    ...
    for on,r in resultats_of.items():
        ...
        for d in r["detail_composants"]:
            ...
            if pd.notna(d["date_prochaine_reception"]) and pd.notna(date_besoin_prod):
                dr_=d["date_prochaine_reception"]
                dbp=date_besoin_prod
                if isinstance(dr_,pd.Timestamp): dr_=dr_.to_pydatetime()
                if isinstance(dbp,pd.Timestamp): dbp=dbp.to_pydatetime()
                delta=(dr_-dbp).days
...
def export_xl(df_cmd,df_det,df_plan,df_crit,params=None):
    ...
    for sn in ["Commandes","Detail Manquants","Plan de Charge","Composants Critiques"]:
        ...
        feu_cols=[h for h in headers if h and ("Feu" in h)]
